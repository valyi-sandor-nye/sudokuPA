...
public class Grid {
    ...
    public Grid(int[][] cells) throws InvalidGridException {
        if (cells == null) {
t        }
        if ( cells.length < ROW_COUNT) {
            throw new InvalidGridException("The grid has to have enough rows!");
        }
        for (int row=0; row<ROW_COUNT;row++)   {
            if( cells[row].length < COLUMN_COUNT)
            throw new InvalidGridException("The grid has not enough cells in th!"+row+"th. row");
        }
